...
class BannerRepository extends GetxController {
  ...
  Future<List<BannerModel>> fetchBanners() async {
    try {
      final result = await THttpHelper.get('banners/');
      final List<dynamic> bannerList = result;
      return bannerList
          .map((bannerJson) =>
              BannerModel.fromJson(bannerJson as Map<String, dynamic>))
          .toList();
  ...
  Future<void> uploadBannersDummyData(List<BannerModel> banners) async {
    try {
      ...
      for (var entry in banners) {
        ...
        final url = await storage.uploadImageData('Banners', thumbnail,
            path.basename(entry.imageUrl), MediaCategory.banners.name);
